...
if(isset($_POST["size"])){// セレクトボックスで選択された値を受け取る
  if(!$_POST["size"] == 0){
    $records = $db->query('select * from sukiya where id="'.$_POST["size"].'"');//入力された商品を呼び出す
...
<h2 class="title">松屋メニュー</h2>
  ...
  <section class="item">
    ...
    <form action="sukiya_item03.php" method = "POST">
      <select name="size" class="size_select">
        <option value="">サイズを選択してください</option>
        <option value = "13">ミニ　420円</option>
        <option value = "14">並盛　480円</option>
        <option value = "15">中盛　610円</option>
        <option value = "16">大盛　610円</option>
        <option value = "17">特盛　760円</option>
        <option value = "18">メガ　910円</option>
      </select><br>
      <input type="submit" name="submit" value="カートに入れる" class="submit_btn">
    </form>
</section>
<div class="menu_but">
  <p><a href="sukiya.php" class="menu">メニューへ戻る</a></p>
</div>

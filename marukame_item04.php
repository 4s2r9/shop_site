...
if(isset($_POST["size"])){// セレクトボックスで選択された値を受け取る
  if(!$_POST["size"] == 0){
    $records = $db->query('select * from marukame where id="'.$_POST["size"].'"');//入力された商品を呼び出す
...
<h2 class="title">丸亀製麺メニュー</h2>
  ...
  <section class="item">
    ...
    <form action="marukame_item04.php" method = "POST">
      <select name="size" class="size_select">
        <option value="">サイズを選択してください</option>
        <option value = "10">並　290円</option>
        <option value = "11">大　400円</option>
        <option value = "12">得　510円</option>
      </select><br>
      <input type="submit" name="submit" value="カートに入れる" class="submit_btn">
    </form>
</section>
<div class="menu_but">
  <p><a href="marukame.php" class="menu">メニューへ戻る</a></p>
</div>

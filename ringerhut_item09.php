...
if(isset($_POST["size"])){// セレクトボックスで選択された値を受け取る
  if(!$_POST["size"] == 0){
    $records = $db->query('select * from ringerhut where id="'.$_POST["size"].'"');//入力された商品を呼び出す
...
<h2 class="title">Ringer Hut メニュー</h2>
  ...
  <section class="item">
    ...
    <form action="ringerhut_item09.php" method = "POST">
      <select name="size" class="size_select">
        <option value="">ぎょうざの個数を選択してください</option>
        <option value = "11">（7個）　370円</option>
        <option value = "12">（10個）　550円</option>
        <option value = "13">（15個）　590円</option>
      </select><br>
      <input type="submit" name="submit" value="カートに入れる" class="submit_btn">
    </form>
</section>
<div class="menu_but">
  <p><a href="ringerhut.php" class="menu">メニューへ戻る</a></p>
</div>

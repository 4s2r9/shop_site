...
if(isset($_POST["size"])){// セレクトボックスで選択された値を受け取る
  if(!$_POST["size"] == 0){
    $records = $db->query('select * from kourakuen where id="'.$_POST["size"].'"');//入力された商品を呼び出す
...
<h2 class="title">幸楽苑メニュー</h2>
  ...
  <section class="item">
    <img src="img/kourakuen_items/item11.jpg" alt="つけめんプレミアム
    ">
    <h1>つけめんプレミアム</h1>

    <form action="kourakuen_item12.php" method = "POST">
      <select name="size" class="size_select">
        <option value = "12">740円</option>
      </select><br>
      <input type="submit" name="submit" value="カートに入れる" class="submit_btn">
    </form>
</section>
<div class="menu_but">
  <p><a href="kourakuen.php" class="menu">メニューへ戻る</a></p>
</div>
